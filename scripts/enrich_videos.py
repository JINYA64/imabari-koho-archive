...
- "summary": 番組が伝えている「今治市の施策・取り組みの内容」を70〜100文字程度、丁寧語（〜です、〜ます）で要約する。
  この文章だけを読んで、今治市が何を実施した（している）のかが端的に伝わるようにすること。
  【禁止】「〜をご紹介します」「〜をご覧ください」「〜をお伝えします」「〜特集」のような、
  動画の視聴を前提にした番組アナウンス的な言い回しは使わないこと。
  【禁止】文頭を毎回「今治市は」で始めないこと。この要約はもともと今治市の広報番組についてのものなので、
  「今治市は」「今治市が」という主語は基本的に省略し、施策や取り組みの内容そのものから文章を始めること
  （例："保育料を無償化し、子育て世帯の負担を軽減しています。" のように、主語なしで自然に読める形にする）。
  代わりに、今治市が実際に行った施策・事業・取り組みそのものを説明する丁寧語の文章にすること。
  例（悪い例。使用禁止）：「今治市の子育て支援策についてご紹介します。」
  例（悪い例。主語の繰り返し）：「今治市は保育料を無償化しています。今治市はさらに相談窓口も設置しています。」
  例（良い例）：「0〜2歳児を対象に保育料を無償化するとともに、子育て世帯向けの相談窓口を新たに設置しています。」

- "plain_title": 元のタイトルから番組回数表記や過剰な装飾を取り除き、内容が一目で直感的にわかる15〜25文字程度のタイトルにする。
  抽象的な言い回し（「〜の取り組み」「〜について」など）よりも、具体的な施策名・地名・対象（誰向けか）を
  優先して盛り込み、読んだ瞬間に「何の話か」がわかるようにすること。
